      ...
      <ol className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {weeks.map((week) => (
          ...
          >
            ...
            <p className="mt-1 text-xs text-slate-600">Trip score: {formatScore(week.trip_score)}</p>
            <p className="mt-1 text-xs text-slate-500">
              Crowd outlook: {getCrowdScoreLabel(week.crowd_score)}
            </p>
            <p className="text-xs text-slate-500">
              Weather outlook: {getWeatherScoreLabel(week.weather_score)}
            </p>
          </li>
        ))}
      </ol>
